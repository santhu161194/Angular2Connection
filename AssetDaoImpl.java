...
public class AssetDaoImpl implements AssetDao {
	...
	public int addAsset(final Asset asset)
		...
		int rows = template.update(Queries.addNewAsset,
	...
	public int updateAssetStatus(final long assetId, final AssetStatus status,
		...
		if (rows1 != 0) {
			rows = template.update(Queries.updateAssetStatus,
					new PreparedStatementSetter() {
						public void setValues(PreparedStatement pst)
								throws SQLException {
	...
	private int updateAssetToLog(long assetId, final String modifiedBy)
		...
		int rows = template.update(Queries.UPDATE_ASSET_TO_LOG_,
				new PreparedStatementSetter() {
	...
	public int saveAssetRequest(final AssetType assetType,
		...
		int rows = template.update(Queries.postAssetRequest,
				new PreparedStatementSetter() {
	...
	public int allocateAsset(final String assignedTo, final long assetId,
		...
		rows = template.update(Queries.allocateAsset,
				new PreparedStatementSetter() {
					public void setValues(PreparedStatement pst)
							throws SQLException {
	...
	private int updateMappingToLog(final long assetId,
		...
		int rows = template.update(Queries.UPDATE_ASSET_MAPPING_TO_LOG,
				new PreparedStatementSetter() {
	...
	public int saveNewAssetTypeRequest(final String requestedBy,
		...
		int rows = 0;
		rows = template.update(Queries.postNewAssetTypeRequest,
				new PreparedStatementSetter() {
					public void setValues(PreparedStatement pst)
							throws SQLException {
						pst.setString(1, requestedBy);
		...
		public int updateAsset(final Asset asset) throws DataAccessException{
			int rows=0;
			int rows1=updateAssetToLog(asset.getAssetId(),asset.getModifiedBy());
			if(rows1!=0)
			{
			rows=template.update(Queries.updateAsset,new PreparedStatementSetter() {
